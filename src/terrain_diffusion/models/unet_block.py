...
from .mp_layers import MPConv, MPConvResample, mp_leaky_relu, mp_silu, mp_sum, normalize, resample
...
class UNetBlock(nn.Module):
    def __init__(
        ...
        if conv_type == "fused" or conv_type == "default":
            self.conv_res0 = MPConv(
                out_channels if mode == "enc" else in_channels,
                out_channels * expansion_factor,
                kernel=[3, 3],
            ...
            self.conv_res0 = nn.ModuleList(
                [
                    ...
                    MPConv(
                        ...
                        out_channels * expansion_factor,
                        kernel=[3, 3],
                        groups=out_channels * expansion_factor,
                        no_padding=no_padding,
                    ),
                ]
            )
        ...
        self.emb_linear = (
            MPConv(emb_channels, out_channels * expansion_factor, kernel=[])
            if emb_channels > 0
            else None
        )
        self.conv_res1 = MPConv(
            out_channels * expansion_factor,
            out_channels,
            kernel=[3, 3] if conv_type == "default" else [1, 1],
            no_padding=no_padding,
        )
        self.conv_skip = (
        ...
        self.attn_qkv = (
            MPConv(out_channels, out_channels * 3, kernel=[1, 1]) if self.num_heads != 0 else None
        )
        self.attn_proj = (
        ...
        if resample_type == "conv" and resample_mode != "keep":
            self.resample = MPConvResample(
                resample_mode,
                kernel=[resample_filter, resample_filter],
    ...
    def attn(self, x):
        ...
        w = torch.einsum(
            "nhcq,nhck->nhqk",
            q,
            k / torch.sqrt(torch.tensor(q.shape[2], dtype=q.dtype, device=q.device)),
        ).softmax(dim=3)
    ...
    def forward(self, x, emb):
        ...
        if x.shape[2:] != y.shape[2:]:
            ...
            x = x[
                :, :, diff_h // 2 : -(diff_h - diff_h // 2), diff_w // 2 : -(diff_w - diff_w // 2)
            ]

...
Copied from the reference implementation of the InfiniteDiffusion paper,
`xandergos/terrain-diffusion` (MIT License, Copyright (c) 2025 Alexander Goslin),
commit e8dcb4b1a834ab2f6b1a6f5256ed7c9f2f3e8230. See LICENSE in this package.
This is the architecture behind the Terrain Diffusion core and decoder models.
"""
...
class EDMUnet2D(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if fourier_scale == "pos":
            self.noise_fourier = (
                MPPositionalEmbedding(noise_emb_dims) if noise_emb_dims > 0 else None
            )
        else:
            self.noise_fourier = (
                MPFourier(noise_emb_dims, s=fourier_scale) if noise_emb_dims > 0 else None
            )
        self.noise_linear = (
            MPConv(noise_emb_dims, emb_channels, kernel=[]) if noise_emb_dims > 0 else None
        )
        ...
        for type, x, weight in conditional_inputs:
            if type == "float":
                self.conditional_layers.append(
                    nn.Sequential(MPFourier(x), MPConv(x, emb_channels, kernel=[]))
                )
            elif type == "tensor":
        ...
        for level, (channels, nb) in enumerate(zip(block_channels, layers_per_block)):
            ...
            if level == 0:
                ...
                self.enc[f"{res}x{res}_down"] = UNetBlock(
                    cout, cout, emb_channels, mode="enc", resample_mode="down", **block_kwargs
                )
            for idx in range(nb):
                cin = cout
                cout = channels
        ...
        for level, (channels, nb) in reversed(
            list(enumerate(zip(block_channels, layers_per_block)))
        ):
            res = image_size // 2**level
            if encode_only:
                continue
            if level == len(block_channels) - 1:
                self.dec[f"{res}x{res}_in0"] = UNetBlock(
                    ...
                    emb_channels,
                    mode="dec",
                    attention=midblock_attention,
                    **block_kwargs,
                )
                self.dec[f"{res}x{res}_in1"] = UNetBlock(
                    cout, cout, emb_channels, mode="dec", **block_kwargs
                )
            else:
                self.dec[f"{res}x{res}_up"] = UNetBlock(
                    cout, cout, emb_channels, mode="dec", resample_mode="up", **block_kwargs
                )
    ...
    def forward(
        self, x, noise_labels, conditional_inputs, return_logvar=False, precomputed_embeds=None
    ):
        conditional_inputs = conditional_inputs or []
        assert len(conditional_inputs) == len(self.conditional_layers), (
            "Invalid number of conditional inputs"
        )

        emb = (
            precomputed_embeds
            if precomputed_embeds is not None
            else self.compute_embeddings(noise_labels, conditional_inputs)
        )
        ...
        if return_logvar:
            logvar = self.logvar_linear(
                self.logvar_fourier(torch.log(torch.tan(noise_labels) / 8))
            ).reshape(-1, 1, 1, 1)
            return x, logvar
        return x

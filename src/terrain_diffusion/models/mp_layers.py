...
def resample(x, mode="keep", factor=2):
    ...
    if mode == "down":
        return torch.nn.functional.conv2d(
            x, torch.ones([c, 1, 1, 1], device=x.device, dtype=x.dtype), groups=c, stride=factor
        )
    if mode == "up_bilinear":
    ...
    return torch.nn.functional.conv_transpose2d(
        x,
        torch.ones([c, 1, factor, factor], device=x.device, dtype=x.dtype),
        groups=c,
        stride=factor,
    )
...
def mp_sum(args, w=None):
    ...
    return torch.sum(
        torch.stack([args * w for args, w in zip(args, w)]), dim=0
    ) / torch.linalg.vector_norm(w)
...
def mp_concat(args, dim=1, w=None):
    ...
    return torch.concat(
        [args[i] * (C / np.sqrt(args[i].shape[dim]) * w[i]) for i in range(len(args))], dim=dim
    )
...
class MPConv(nn.Module):
    ...
    def forward(self, x, gain=1):
        ...
        return nn.functional.conv2d(
            x, w, padding=(0 if self.no_padding else w.shape[-1] // 2,), groups=self.groups
        )
...
class MPEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        assert torch.max(x) < self.weight.shape[0], (
            f"Embedding index out of bounds: {torch.max(x).item()}"
        )

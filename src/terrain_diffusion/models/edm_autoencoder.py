...
class EDMAutoencoder(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.decoder_conv = MPConv(
            latent_channels + len(direct_skips) + 1,
            model_channels * model_channel_mults[-1],
            kernel=[1, 1],
        )
        cout = (
            model_channels * model_channel_mults[-1]
        )  # +1 because we add a ones channel to simulate a bias
        for level, (channels, nb) in reversed(
            list(enumerate(zip(block_channels, layers_per_block_decoder)))
        ):
            res = image_size // 2**level
            if level == len(block_channels) - 1:
                self.decoder.append(
                    UNetBlock(
                        cout, cout, 0, mode="dec", attention=midblock_attention, **block_kwargs
                    )
                )
                self.decoder.append(UNetBlock(cout, cout, 0, mode="dec", **block_kwargs))
            else:
                self.decoder.append(
                    UNetBlock(cout, cout, 0, mode="dec", resample_mode="up", **block_kwargs)
                )
    ...
    def preencode(self, x, conditional_inputs=None):
        ...
        final_logvars = torch.cat(
            [
                logvars,
                torch.full(
                    [
                        logvars.shape[0],
                        len(self.config.direct_skips),
                        logvars.shape[2],
                        logvars.shape[3],
                    ],
                    device=logvars.device,
                    fill_value=-20,
                ),
            ],
            dim=1,
    ...
    def decode(self, z, include_logvar=False):
        ...
        if len(self.config.direct_skips) > 0:
            ...
            for i, channel_idx in enumerate(self.config.direct_skips):
                direct_output[:, channel_idx : channel_idx + 1] = torch.nn.functional.interpolate(
                    direct_channels[:, i : i + 1], size=direct_output.shape[-2:], mode="nearest"
                )
                direct_output_mask[:, channel_idx : channel_idx + 1] = 1

            decoder_out = (
                direct_output_mask * direct_output + (1 - direct_output_mask) * decoder_out
            )
